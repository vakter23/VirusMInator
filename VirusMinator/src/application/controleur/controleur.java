	...
	public static void main(String[] args) {
		...
		for (int i = 0; i <tabMap.length; i++) {
			for (int j = 0; j <tabMap[i].length; j++) {
				...
				if(listeMap.get(i*j)==BordTerrain) {
					tabMap[i][j]= "BordTerrain";
				}
					
				else if(listeMap.get(i*j)==Herbe) {
					tabMap[i][j]= "Herbe";

				}
				
				else if(listeMap.get(i*j)==Sable) {
					tabMap[i][j]= "Sable";

				}
				
				else if(listeMap.get(i*j)==BlocVioletSpawnMobSurement) {
					tabMap[i][j]= "BlocVioletSpawnMobSurement";

				}
				
				
				else if(listeMap.get(i*j)==BlocVertSpawnMob) {
					tabMap[i][j]= "BlocVertSpawnMob";

				}
				
				else if(listeMap.get(i*j)==Chemin) {
					tabMap[i][j]= "Chemin";

				}
				
				else if(listeMap.get(i*j)==BlocBlancHopital) {
					tabMap[i][j]= "BlocBlancHopital";

				}
				
				else if(listeMap.get(i*j)==PlacementTourelles) {
					tabMap[i][j]= "PlacementTourelles";

				}
				
				else if(listeMap.get(i*j)==BlocRougeHopital) {
					tabMap[i][j]= "BlocRougeHopital";

				}
				
				else if(listeMap.get(i*j)==BLocBasDroiteArriveMob) {
					tabMap[i][j]= "BLocBasDroiteArriveMob";

				}
				
				else if(listeMap.get(i*j)==BlocSpawnMob) {
